...
void yyeva::ELog::log(string level, string tagName, char *format, va_list args)
{
    ...
    if (level == "i") {
        __ohos_log_print(IL_INFO, tagName.c_str(), buf);
    } else if (level == "d") {
        __ohos_log_print(IL_DEBUG, tagName.c_str(), buf);
    } else if (level == "e") {
        __ohos_log_print(IL_ERROR, tagName.c_str(), buf);
    } else {
        __ohos_log_print(IL_INFO, tagName.c_str(), buf);
    }
...
void yyeva::ELog::log(string level, string tagName, string msg)
{
    ...
    std::vector<napi_value> args = {napiHandler.GetNapiValue<std::string>(level),
        napiHandler.GetNapiValue<std::string>(tagName), napiHandler.GetNapiValue<std::string>(msg)};
    napiHandler.QueueWork(elog, args);
}
#else
void yyeva::ELog::log(string level, string tagName, string msg)
{
    ...
    jmethodID methodId = env->GetMethodID(clazz_listener, level.c_str(), "(Ljava/lang/String;Ljava/lang/String;)V");
