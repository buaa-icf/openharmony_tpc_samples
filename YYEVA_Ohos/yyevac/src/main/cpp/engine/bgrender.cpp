...
#else
void yyeva::BgRender::setBgImage(unsigned char *bitmap, OH_NativeBuffer_Config *bitmapInfo) {
#endif
    textureId = TextureLoadUtil::loadTexture(bitmap, bitmapInfo);
}

void yyeva::BgRender::initRender() {
    char VERTEX_SHADER[] = R"(#version 320 es
        layout(location = 0) in vec4 vPosition;
        layout(location = 1) in vec4 vTexCoordinate;
    ...
    char FRAGMENT_SHADER[] = R"(#version 320 es
