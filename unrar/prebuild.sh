...
UNRAR_NAME=unrar                                                            # 依赖库名

CI_OUTPUT_DIR=$ROOT_DIR/../out/tpc/                                         # hap/har安装目录

LIBS_NAME=("unrar")
PACKAGE_NAME=("unrar.zip")
...
{
    copy_depends $DEPENDS_DIR $UNRAR_NAME
}
...
{
    check_copy_shasum $UNRAR_NAME ${PACKAGE_NAME[0]} $UNRAR_NAME
}
...
{
    local install_dir=$ROOT_DIR/library/src/main/cpp/third_party/
    mkdir $ROOT_DIR/library/src/main/cpp/third_party/
    cp -arf $LYCIUM_TOOLS_DIR/usr/$UNRAR_NAME $install_dir/$UNRAR_NAME
    if [ $? -ne 0 ]
    then
        echo "unrar build failed!"
        return 1
    fi

    if [ -d $CI_OUTPUT_DIR ]
    then
        cp -arf $LYCIUM_TOOLS_DIR/usr/$UNRAR_NAME $CI_OUTPUT_DIR
    fi
