...
static lua_State *GetLuaStateL() { return g_L; }
...
static int l2tLoadModule(lua_State *L) {
    ...
    DLOGINFO("testTag", "读取模块,结果=%{public}d,pathModuleTs=%{public}s", status, pathModuleTs.c_str());
...
static int l2tCallModuleFunc(lua_State *L) {
    ...
    string pathModuleTs = lua_tostring(L, 1);
    DLOGINFO("testTag", "l2tCallModuleFunc,获取lua参数,pathModuleTs=%{public}s", pathModuleTs.c_str());

    string strFunction = lua_tostring(L, 2);
    DLOGINFO("testTag", "l2tCallModuleFunc,获取lua参数,pathModuleTs=%{public}s", strFunction.c_str());

    napi_value result;
    napi_status status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tCallModuleFunc,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tSetModuleVarInt(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tSetModuleVarInt,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tSetModuleVarInt,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tSetModuleVarLong(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tSetModuleVarLong,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());

    string pathModuleTs = lua_tostring(L, 1);
    DLOGINFO("testTag", "l2tSetModuleVarLong,获取lua参数,pathModuleTs=%{public}s", pathModuleTs.c_str());
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tSetModuleVarLong,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tSetModuleVarChar(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tSetModuleVarChar,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tSetModuleVarChar,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tSetModuleVarDouble(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tSetModuleVarDouble,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tSetModuleVarDouble,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tSetModuleVarBool(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tSetModuleVarBool,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tSetModuleVarBool,读取模块,结果=%{public}d, pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tGetModuleVarInt(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tGetModuleVarInt,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());

    string pathModuleTs = lua_tostring(L, 1);
    DLOGINFO("testTag", "l2tGetModuleVarInt,获取lua参数,pathModuleTs=%{public}s", pathModuleTs.c_str());

    string keyStr = lua_tostring(L, 2);
    DLOGINFO("testTag", "l2tGetModuleVarInt,获取lua参数,keyStr=%{public}s", keyStr.c_str());

    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tGetModuleVarInt,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tGetModuleVarLong(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tGetModuleVarLong,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    string pathModuleTs = lua_tostring(L, 1);
    DLOGINFO("testTag", "l2tGetModuleVarLong,获取lua参数,pathModuleTs=%{public}s", pathModuleTs.c_str());

    string keyStr = lua_tostring(L, 2);
    DLOGINFO("testTag", "l2tGetModuleVarLong,获取lua参数,keyStr=%{public}s", keyStr.c_str());

    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tGetModuleVarLong,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
    ...
    if (status == napi_ok) {
        DLOGINFO("testTag", "l2tGetModuleVarLong,获取long变量成功, value= %{public}ld", tmpVal);
    } else {
        DLOGINFO("testTag", "l2tGetModuleVarLong,获取long变量失败, status= %{public}d", status);
    }
...
static int l2tGetModuleVarChar(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tGetModuleVarChar,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    string pathModuleTs = lua_tostring(L, 1);
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tGetModuleVarChar,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tGetModuleVarDouble(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tGetModuleVarDouble,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());
    string pathModuleTs = lua_tostring(L, 1);
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tGetModuleVarDouble,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
...
static int l2tGetModuleVarBool(lua_State *L) {
    if (g_bLog)
        OH_LOG_Print(LOG_APP, LOG_INFO, 0, "testTag l2tGetModuleVarBool,触发lua动态加载ts模块",
                     "func=%{public}s,line=%{public}d,file=%{public}s", __FUNCTION__, __LINE__,
                     GetFileName(__FILE__).c_str());

    string pathModuleTs = lua_tostring(L, 1);
    DLOGINFO("testTag", "l2tGetModuleVarBool,获取lua参数,pathModuleTs=%{public}s", pathModuleTs.c_str());
    ...
    napi_value result;
    auto status = LoadModule(pathModuleTs, &result);
    DLOGINFO("testTag", "l2tGetModuleVarBool,读取模块,结果=%{public}d,pathModuleTs=%{public}s", status,
             pathModuleTs.c_str());
    ...
    if (status == napi_ok) {
        DLOGINFO("testTag", "l2tGetModuleVarBool,获取bool类型变量成功, value= %{public}d", tmpBool);
    } else {
        DLOGINFO("testTag", "l2tGetModuleVarBool,获取bool类型变量失败, status= %{public}d", status);
    }
