...
```typescript
const emitter = new EventEmitter(); //初始化EventEmitter

emitter.on('event', (name, age) => {   //绑定事件event
...
struct EventSequencing {
  ...
  @State emitter: EventEmitter = undefined

  aboutToAppear() {
    this.emitter = new EventEmitter();
    this.emitter.on('event', (name, age, message) => {}); //页面初始化的时候添加同名的event事件
  }

  onPageShow() {
    this.emitter.on('event', (name, age, message) => {}); //页面显示的时候添加同名的event事件
  }
...
在下述版本验证通过：
DevEco Studio: 4.0 Beta2(4.0.3.501), SDK: API10 Beta3(4.0.10.8)
