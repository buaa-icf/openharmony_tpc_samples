...
> socketio_tls is a library that implements low-latency, two-way, and event-based communication between clients and servers. It is built on top of the WebSocket protocol and ensures communication security through TLS support provided by OpenSSL. It also offers additional features such as falling back to HTTPS long polling and automatic reconnection.
### Precautions
- socket_tls supports only HTTPS (TLS) connections and does not support HTTP.
- To establish a connection via HTTP, please use the [socketio](https://gitcode.com/openharmony-tpc/openharmony_tpc_samples/tree/master/socketio) version.
- The two versions are respectively suitable for HTTPS and HTTP scenarios and **must not be mixed**.
### Server Compatibility
- **Compatible server versions: Socket.IO 3.x and 4.x**

## Effect

<img src="screenshot/effect_login.jpg" alt="avatar" style="zoom:50%;" />

<img src="screenshot/effect_chat.jpg" alt="avatar" style="zoom:50%;" />

## How to Install

```shell
ohpm install @ohos/socketio_tls
```
...
```typescript
import { client_socket } from '@ohos/socketio_tls';
```
