...
type Test = [
  ...
  Array<[any, string | null, TokensToFunctionOptions?]>
];
...
export const fn = match("/user/:id", {
  decode: decodeURIComponent
});

export const fn1 = match("/café", {
  encode: encodeURI
});

export const urlMatch = match("/users/:id/:tab(home|photos|bio)", {
  decode: decodeURIComponent,
});

export const toPath = compile("/user/:id", {
  encode: encodeURIComponent
});
 ...
 * @return {Array}
 */
export function exec(re: RegExp, str: string) {
  const match = re.exec(str);

  return match && Array.prototype.slice.call(match);
}

export function ToPath(path, opts, options) {
  return compile(path, {
    ...opts,
    ...options,
  });
