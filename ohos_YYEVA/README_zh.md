...
build() {
	Column() {
		EvaAnimComponent({ playerEva: this.player, hitTestBehaviorMode: HitTestMode.Default })
			.width('100%')
			.height('80%');
...
### 核心类说明
#### EvaAnimPlayer：
`EvaAnimPlayer` 是播放控制器，负责加载、播放、暂停、释放资源等完整生命周期管理。

##### 属性

| 属性名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `isNormalMp4` | boolean | 否 | 是否为不透明 MP4，默认 `false`。 |
| `playLoop` | number | 否 | 循环播放次数（通过 setter 同步到解码器）。 |
| `isLoop` | boolean | 否 | 是否无限循环（通过 setter 同步到解码器）。 |
| `isSetLastFrame` | boolean | 否 | 是否停留在最后一帧，默认 `false`。 |
| `videoMode` | number | 否 | 视频模式，默认 `EvaConstant.VIDEO_MODE_NORMAL_MP4`。 |
| `blendMode` | number | 否 | 混合模式，默认 `EvaBlendMode.BLEND_SRC_ALPHA`。 |


###
#### EvaAnimComponent：
`EvaAnimComponent` 是承载播放器渲染画布并绑定 `EvaAnimPlayer` 的 ArkTS UI 组件。

##### 属性

| 属性名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `playerEva` | `IEvaAnimPlayer` | 是 | 绑定播放器实例，用于驱动渲染与播放控制。 |
| `xcompID` | string | 否 | XComponent 组件 ID；为空时会在 `aboutToAppear` 中自动生成 `EvaAnimComponent + uuid`。 |
| `xType` | `XComponentType` | 否 | XComponent 类型，默认 `XComponentType.SURFACE`。 |
| `evaView` | `EvaAnimView` | 否 | 组件内部使用的控制器实例，默认 `new EvaAnimView()`。 |
| `hitTestBehaviorMode` | `HitTestMode` | 否 | 命中测试行为，默认 `HitTestMode.Default`。 |

###
#### IEvaFetchResource：
`IEvaFetchResource` 是动态资源获取接口，用于在渲染前替换图片、文本等资源。

##### 方法

| 方法名 | 签名 | 必填 | 说明 |
| --- | --- | --- | --- |
| `setImage` | `(resource: EvaResource, result: (bitmap: image.PixelMap \| null, fitType: EvaSrc.FitType \| null) => void) => void` | 是 | 获取图片资源并通过回调返回 PixelMap 和适配方式。 |
| `setText` | `(resource: EvaResource, result: (resource: EvaResource) => void) => void` | 是 | 获取文字资源并通过回调返回更新后的 EvaResource。 |
| `releaseSrc` | `(resources: EvaResource[]) => void` | 是 | 资源释放通知，用于清理缓存等。 |


###
#### OnEvaResourceClickListener：
`OnEvaResourceClickListener` 是资源点击监听接口，用于处理动画内资源的点击事件。

##### 方法

| 方法名 | 签名 | 必填 | 说明 |
| --- | --- | --- | --- |
| `onClick` | `(resource: EvaResource) => void` | 是 | 点击资源回调，返回被点击的资源信息。 |


###
#### EvaResource：
`EvaResource` 是资源描述类，用于承载可替换资源的元信息。
...
| --- | --- | --- | --- |
| `id` | string | 否 | 资源 ID。 |
| `type` | `EvaSrc.SrcType` | 否 | 资源类型，默认 `EvaSrc.SrcType.UNKNOWN`。 |
| `loadType` | `EvaSrc.LoadType` | 否 | 加载类型，默认 `EvaSrc.LoadType.UNKNOWN`。 |
| `tag` | string | 否 | 资源标签，用于匹配替换。 |
| `scaleMode` | string | 否 | 缩放模式，示例：`scaleFill`、`aspectFit`、`aspectFill`。 |
| `bitmap` | `image.PixelMap \| null` | 否 | 图片资源 PixelMap。 |
| `text` | `string \| null` | 否 | 文本内容。 |
| `textColor` | number | 否 | 文本颜色，默认 `-1`。 |
| `fontSize` | number | 否 | 文本字号。 |
| `textAlign` | `string \| null` | 否 | 文本对齐方式。 |
| `typeFace` | `string \| null` | 否 | 字体样式，字体族名。 |

##### 方法

| 方法名 | 签名 | 必填 | 说明 |
| --- | --- | --- | --- |
| `constructor` | `(src: EvaSrc) => EvaResource` | 是 | 由 `EvaSrc` 构造资源描述实例。 |
...
本库基于 OpenHarmony API 18 (SDK 5.1.0)开发，在 API18 环境以下存在以下功能限制：
