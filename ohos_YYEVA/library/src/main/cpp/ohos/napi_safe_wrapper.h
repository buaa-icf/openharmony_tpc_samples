...
template <typename T>
class NapiSafeWrappable : public std::enable_shared_from_this<NapiSafeWrappable<T>> {
private:
...
public:
    NapiSafeWrappable(std::shared_ptr<T> object) : nativeObject_(std::move(object)) {}

    template <typename... Args>
    NapiSafeWrappable(Args &&...args) : nativeObject_(std::make_shared<T>(std::forward<Args>(args)...))
    {
    }

    virtual ~NapiSafeWrappable()
    {
        invalidate();
    }
    ...
    auto ExecuteSafely(Func &&func) -> decltype(func(std::declval<std::shared_ptr<T>>()))
    {
        auto obj = GetSafeObject();
        if (!obj) {
