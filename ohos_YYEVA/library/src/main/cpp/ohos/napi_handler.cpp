...
static std::unordered_map<int32_t, std::shared_ptr<NapiObject>> objectMap_;
...
napi_value NapiHandler::BindSafeObject(const std::shared_ptr<NapiObject> &object, napi_finalize destructor)
{
    ...
    NAPI_CALL(env_, napi_wrap(env_, thisArg_, static_cast<void *>(object.get()), destructor, nullptr, nullptr));
    objectMap_[id] = object;
    return thisArg_;
}

std::shared_ptr<NapiObject> NapiHandler::UnbindSafeObject()
{
    NapiObject *t = nullptr;
...
void NapiHandler::UnrefSafeObject(void *ptr)
{
    int32_t id = static_cast<int32_t>(reinterpret_cast<intptr_t>(ptr));
    if (objectMap_.find(id) == objectMap_.end()) {
...
std::shared_ptr<NapiObject> NapiHandler::GetSafeObject(void *ptr)
{
    int32_t id = static_cast<int32_t>(reinterpret_cast<intptr_t>(ptr));
    if (objectMap_.find(id) == objectMap_.end()) {
        LOGE("not find object, %d", id);
