...
void yyeva::EvaMixRender::rendFrame(
    GLuint videoTextureId, shared_ptr<EvaAnimeConfig> config, shared_ptr<EvaFrame> frame, shared_ptr<EvaSrc> src)
{
    if (videoTextureId <= 0) {
    ...
    // src纹理坐标 坐标归一
    float *array = genSrcCoordsArray(
        srcArray->array, frame->frame->w, frame->frame->h, src->bitmapWidth, src->bitmapHeight, src->fitType);
    ...
    // mask纹理 只有r通道为透明值
    maskArray->setArray(
        TexCoordsUtil::create(config->videoWidth, config->videoHeight, frame->mFrame, maskArray->array));
    if (frame->mt == 90) {
 ...
 * @return
 */
float *yyeva::EvaMixRender::genSrcCoordsArray(
    float *array, float fw, float fh, float sw, float sh, EvaSrc::FitType fitType)
{
