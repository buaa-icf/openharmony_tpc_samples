...
#ifdef __OHOS__
#define ANativeWindow_getWidth  ANativeWindowGetWidth
#define ANativeWindow_getHeight ANativeWindowGetHeight
#endif
...
void yyeva::RenderController::mixConfigCreate(shared_ptr<EvaAnimeConfig> config) {
    if (config == nullptr) {
        ELOGE("mix config create failed, config is nullptr");
        return;
    } else {
        this->config = config;
    }
    if (!config->isMix) {
        ELOGE("config is not Mix");
        return;
    }
    ELOGE("mixConfigCreate ddd");
    parseSrc(config);
    parseFrame(config);
}
...
#ifdef __OHOS__
void yyeva::RenderController::setSrcBitmap(const char *srcId, unsigned char *bitmap, OH_NativeBuffer_Config *bitmapInfo,
                          string scaleMode) {
#else
void yyeva::RenderController::setSrcBitmap(const char *srcId, unsigned char *bitmap,
    ...
    if (srcMap == nullptr) {
        ELOGE("srcMap is null, maybe the render not init");
        return;
    }
    if (srcMap->map.empty()) {
        ELOGE("not render in map, id: %s", srcId);
        return;
    }
...
#ifdef __OHOS__
        if (bitmapInfo->format == NATIVEBUFFER_PIXEL_FMT_RGB_565) { // RGB三通道，例如jpg格式
#else
...
#ifdef __OHOS__
        } else if (bitmapInfo->format == NATIVEBUFFER_PIXEL_FMT_RGBA_8888) { 
#else
...
#ifdef __OHOS__
void yyeva::RenderController::setBgImage(unsigned char *bitmap, OH_NativeBuffer_Config *bitmapInfo) {
#else
