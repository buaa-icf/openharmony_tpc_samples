...
int32_t Demuxer::Create(SampleInfo &info)
{
    ...
    if (!info.inputFilePath.empty()) {
        source_ = OH_AVSource_CreateWithURI((char *)(info.inputFilePath.c_str()));
    } else {
...
{
    OH_AVFormat_GetIntValue(sourceFormat.get(), OH_MD_KEY_TRACK_COUNT, &trackCount_);
    if (!OH_AVFormat_GetLongValue(sourceFormat.get(), OH_MD_KEY_DURATION, &info.duration)) {
        LOGD("get duration failed");
    }
    LOGD("get duration: %lld", info.duration);
...
void Demuxer::SelectAudioTrack(int32_t index, SampleInfo &info)
{
    auto trackFormat = GetTrackFormat(index);
    int trackType = GetTrackType(trackFormat);
    ProcessAudioTrack(trackFormat, index, info);
...
void Demuxer::SelectVideoTrack(int32_t index, SampleInfo &info)
{
    auto trackFormat = GetTrackFormat(index);
    int trackType = GetTrackType(trackFormat);
    ProcessVideoTrack(trackFormat, index, info);
...
OH_AVFormat *Demuxer::GetTrackFormatP(int32_t index)
{
    return OH_AVSource_GetTrackFormat(source_, index);
}
...
void Demuxer::LogAudioConfig(const SampleInfo &info, const char *audioCodecMime)
{
    ...
    AVCODEC_SAMPLE_LOGI("audioMime:%{public}s sampleForamt:%{public}d sampleRate:%{public}d "
        ...
        audioCodecMime, info.audioSampleForamt, info.audioSampleRate, info.audioChannelCount, info.audioChannelLayout,
        info.aacAdts);
    AVCODEC_SAMPLE_LOGI("====== Demuxer Audio config ======");
}
...
void Demuxer::LogCodecConfigDetails(const SampleInfo &info)
{
    AVCODEC_SAMPLE_LOGI("codecConfig:%{public}p, len:%{public}i, 0:0x%{public}02x 1:0x:%{public}02x, bufLen:%{public}u",
        info.codecConfig, static_cast<int>(info.codecConfigLen), info.codecConfig[0], info.codecConfig[1],
...
int32_t Demuxer::Seek(int64_t position, OH_AVSeekMode mode)
{
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    if (demuxer_ == nullptr) {
        AVCODEC_SAMPLE_LOGE("demuxer is null, Seek failed");
        return AVCODEC_SAMPLE_ERR_ERROR;
    }
    int32_t ret = OH_AVDemuxer_SeekToTime(demuxer_, position, mode);
    CHECK_AND_RETURN_RET_LOG(ret == AV_ERR_OK, AVCODEC_SAMPLE_ERR_ERROR, "Seek failed");
    return AVCODEC_SAMPLE_ERR_OK;
}

int32_t Demuxer::GetTrackCount()
{
    return trackCount_;
}

int64_t Demuxer::GetTimestamp()
{
    return timestamp_;
}
