...
int32_t VideoDecoder::SetCallback(CodecUserData *codecUserData)
{
    ...
    ret = OH_VideoDecoder_RegisterCallback(
        decoder_,
        {SampleCallback::OnCodecError, SampleCallback::OnCodecFormatChange, SampleCallback::OnNeedInputBuffer,
         SampleCallback::OnNewOutputBuffer},
        codecUserData);
...
int32_t VideoDecoder::Configure(const SampleInfo &sampleInfo)
{
    ...
    AVCODEC_SAMPLE_LOGI(
        "%{public}d*%{public}d, %{public}.1ffps", sampleInfo.videoWidth, sampleInfo.videoHeight, sampleInfo.frameRate);
    AVCODEC_SAMPLE_LOGI("====== VideoDecoder config ======");
...
int32_t VideoDecoder::Config(const SampleInfo &sampleInfo, CodecUserData *codecUserData)
{
    ...
    if (sampleInfo.window != nullptr) {
        ...
        CHECK_AND_RETURN_RET_LOG(
            ret == AV_ERR_OK && sampleInfo.window, AVCODEC_SAMPLE_ERR_ERROR, "Set surface failed, ret: %{public}d",
            ret);
    }
