...
public:
    ...
    int32_t Seek(int64_t position, OH_AVSeekMode mode = SEEK_MODE_PREVIOUS_SYNC);
    int32_t GetTrackCount();
    std::shared_ptr<OH_AVFormat> GetTrackFormat(int32_t index);
    OH_AVFormat *GetTrackFormatP(int32_t index);
    void SelectAudioTrack(int32_t index, SampleInfo &info);
    void SelectVideoTrack(int32_t index, SampleInfo &info);
