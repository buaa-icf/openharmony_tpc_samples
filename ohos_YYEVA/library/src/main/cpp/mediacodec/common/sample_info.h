...
enum CodecRunMode { SURFACE = 0, BUFFER = 1 };
...
public:
    ...
    void WriteCache(void *buffer, int32_t bufferLen)
    {
        ...
        std::copy(static_cast<uint8_t *>(buffer), static_cast<uint8_t *>(buffer) + bufferLen, cache.data() + remainlen);
        remainlen += bufferLen;
    }
    ...
    bool ReadCache(void *buffer, int32_t bufferLen)
    {
        ...
        std::copy(
            static_cast<const char *>(cache.data()), static_cast<const char *>(cache.data()) + bufferLen,
            static_cast<char *>(buffer));
        remainlen = remainlen - bufferLen;
        if (remainlen > 0) {
            std::copy(cache.begin() + bufferLen, cache.begin() + bufferLen + remainlen, cache.begin());
        }
        return true;
    }
};
