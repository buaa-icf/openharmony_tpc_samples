...
export default class NoTlsUtil {
  ...
  public async doLogin(options: AccessOptions, listener?: LoginListener) {
    try {
      ...
      if (listener) {
        ...
        console.log("FTPClientTest : closed 获取发客户端状态 : " + clientStatus)
  ...
  public async uploadSingleFile(localPath: string, remotePath: string, listener?: UploadSingleListener) {
    ...
    try {
      if (listener) {
        ...
        if (!stat) {
          listener.uploadErr(new Error('获取本地文件信息失败，上传流程结束'))
          return;
        }
        ...
        if (!response) {
          this.client.trackProgress(null)
          listener.uploadErr(new Error('上传单个文件失败，未获得响应信息'))
          return;
        }
        this.client.trackProgress(null)
        ...
        if (!stat) {
          throw new Error('获取本地文件信息失败，上传流程结束')
        }
        let totalSize = stat.size;
        this.client.trackProgress((info) => {
          if (listener) {
        ...
        if (!response) {
          this.client.trackProgress(null)
          throw new Error('上传单个文件失败，未获得响应信息')
        }
        this.client.trackProgress(null)
  ...
  public async appendFile(localPath: string, remotePath: string, listener?: AppendListener) {
    ...
    try {
      if (listener) {
        ...
        if (!stat) {
          listener.appendErr(new Error('获取本地文件信息失败，上传流程结束'))
          return;
        }
        let totalSize = stat.size;
        this.client.trackProgress((info) => {
        ...
        listener.appendStart('开始附加上传单个文件')
        ...
        if (!response) {
          this.client.trackProgress(null)
          listener.appendErr(new Error('附加上传单个文件失败，未获得响应信息'))
          return;
        }
        this.client.trackProgress(null)
        ...
        console.log(`${this.TAG}------>${'开始附加上传单个文件'}`)
        let [listErr, response] = await to<FTPResponse>(this.client.uploadFrom(localPath, remotePath))
        if (listErr) {
          this.client.trackProgress(null)
          throw listErr
        ...
        if (!response) {
          this.client.trackProgress(null)
          throw new Error('附加上传单个文件失败，未获得响应信息')
        }
        this.client.trackProgress(null)
        listener.appendSuccess(response)
        console.log(`${this.TAG}---uploadFrom success--->${JSON.stringify(response)}`)
  ...
  public async uploadDir(localDir: string, remoteDir: string, listener?: UploadDirListener) {
    ...
    try {
      if (listener) {
        ...
        listener.uploadDirStart('开始上传文件夹')
        ...
        this.client.trackProgress(null)
        listener.uploadDirSuccess('上传文件夹成功')
      } else {
        if (!this.client) {
          throw new Error('client object not init yet')
        }
        ...
        this.client.trackProgress(null)
        console.log(`${this.TAG}---uploadFromDir success--->${'上传文件夹成功'}`)
      }
  ...
  public async getTotalSize(cacheList: LinkedList<number>, localDir: string, listener?: UploadDirListener): Promise<number> {
    ...
    for (const file of files) {
      ...
      if (!stats) {
        if (listener) listener.uploadDirErr(new Error('获取本地文件信息失败，上传流程结束'))
        throw new Error('获取本地文件信息失败，上传流程结束');
      }
      if (stats.isFile()) {
