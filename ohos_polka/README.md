...
### 三、大文件下载
- 支持大文件下载。
- 采用分块传输（Chunked Transfer Encoding）方式：Transfer-Encoding=chunked。
- 参考使用说明中的'/download'中间件，建议大于10MB文件场景时使用。
...
app
  ...
  // 下载文件中间件，调用res.end()方法，参数包含要下载的文件沙箱路径、下载名称、request请求头
...
This project is licensed under [MIT License](https://gitcode.com/openharmony-tpc/openharmony_tpc_samples/blob/master/ohos_polka/LICENSE).
