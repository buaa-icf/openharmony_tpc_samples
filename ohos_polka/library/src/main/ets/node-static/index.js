...
import { path, events, process, Url } from "@ohos/node-polyfill";

import { mime } from './mime';
export { mime } from './mime';
import { mstat } from './node-static/util';
import { getListFile } from '../common/util/fs';

export class Server {
    constructor (context, root, options) {
    ...
    tryStat(p, callback) {
        try {
            fs.stat(this.pathDir + p, callback);
        } catch (e) {
            callback(e);
        }
    }
    ...
    serveDir (pathname, req, res, finish) {
        const htmlIndex = path.join(pathname, this.options.indexFile), that = this;

        this.tryStat(htmlIndex, function (e, stat) {
            if (!e) {
                const status = 200;
                const headers = {};
                const originalPathname = decodeURIComponent(new Url(req.url, 'http://localhost').pathname); // 待测试，看是否报错
                ...
                res.end(that.listDirectory(that.pathDir, pathname.substring(that.root.length)));
            }
        });
    ...
    listDirectory(pathDir, uri) {
        let heading = "Directory " + uri;
        let msg =
        ...
        let up = '';
        if (uri.length > 1) {
        let u = uri.substring(0, uri.length - 1);
        ...
        let files = getListFile(pathDir + this.root + uri).files
        // sort(files);
        let directories = getListFile(pathDir + this.root + uri).directories
        // sort(directories);
        if (up !== '' || directories.length + files.length > 0) {
            msg += "<ul>";
            if (up != null || directories.length > 0) {
                msg += "<section class=\"directories\">";
                if (up != null) {
                    msg += ("<li><a rel=\"directory\" href=\"" + up + "\"><span class=\"dirname\">..</span></a></li>");
                }
                directories.forEach((directory) => {
                    let dir = directory + "/";
                    msg += ("<li><a rel=\"directory\" href=\"" + encodeURI(dir) + "\"><span class=\"dirname\">" + dir + "</span></a></li>");
                })
                msg += "</section>";
            }
            if (files.length > 0) {
                ...
                files.forEach((file) => {
                    msg += ("<li><a href=\"" + encodeURI(file) + "\"><span class=\"filename\">" + file + "</span></a>");
                    msg += "</li>";
                })
                msg += "</section>";
            }
    ...
    serveFile (pathname, status, headers, req, res) {
        ...
        this.tryStat(pathname, function (e, stat) {
            if (e) {
                return promise.emit('error', e);
            }
            that.respond(null, status, headers, [pathname], stat, req, res, function (status, headers) {
                that.finish(status, headers, req, res, promise, null);
            });
        });
        return promise;
    }
    ...
    servePath (pathname, status, headers, req, res, finish) {
        ...
        if (pathname.startsWith(that.root)) {
            that.tryStat(pathname, function (e, stat) {
                if (e) {
                    // possibly not found, check default extension
                    if (that.defaultExtension) {
                        this.tryStat(pathname + that.defaultExtension, function(e2, stat2) {
                            if (e2) {
                                // really not found
                                finish(404, {});
    ...
    serve (req, res, callback) {
        ...
        try {
            pathname = decodeURIComponent(new Url(req.url, 'http://localhost').pathname);   // 待测试，看是否报错
        } catch(e) {
    ...
    respondGzip (pathname, status, contentType, _headers, files, stat, req, res, finish) {
        ...
        if (files.length == 1 && this.gzipOk(req, contentType)) {
            ...
            this.tryStat(gzFile, function (e, gzStat) {
                if (!e && gzStat.isFile()) {
                ...
                that.respondNoGzip(pathname, status, contentType, _headers, files, stat, req, res, finish, gzStat && gzStat.size);
            });
        } else {
    ...
    respondNoGzip (pathname, status, contentType, _headers, files, stat, req, res, finish, gzSize) {
        const mtime           = stat.mtime * 1000,
        ...
        if (!byteRange.valid && req.headers['range']) {
            console.error('Range request present but invalid, might serve whole file instead');
        }

        // Copy default headers
        for (const k in this.options.headers) {  headers[k] = this.options.headers[k] }

        headers['Etag']          = [stat.ino, stat.size, mtime].join('-');
        headers['Date']          = new(Date)().toUTCString();
        headers['Last-Modified'] = new(Date)(stat.mtime * 1000).toUTCString();
    ...
    respond (pathname, status, _headers, files, stat, req, res, finish) {
        const contentType = _headers['Content-Type'] || mime.getType(files[0].substring(files[0].lastIndexOf('/'))) || 'application/octet-stream';
    ...
    stream (pathname, files, length, startByte, res, callback) {
        ...
        (function streamFile(files, offset) {
            ...
            if (file) {
                ...
                let ss = fs.createStreamSync(that.pathDir + file, 'r+');
                let buf = new ArrayBuffer(length);
                let readOptions = {
                    offset: startByte,
                    length: length
                }
                ss.read(buf, readOptions, (err, readLen) => {
                    if (err) {
                        callback(err);
                        console.error("read stream failed with error message: " + err.message + ", error code: " + err.code);
                    } else {
                        console.info("read data succeed");
                        if (readLen && offset < length && offset >= 0) {
                            offset += readLen;
                        }
                        ss.close((err) => {
                            if (err) {
