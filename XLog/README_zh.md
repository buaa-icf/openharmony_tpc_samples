...
| `mode`          | Appender模式。<br> APPENDER_MODE_SYNC: 同步模式,立即保存日志到文件； <br>APPENDER_MODE_SYNC: 异步模式,日志不会立即保存，而是每隔15分钟自动Flush一次。                                                                           |
| `logdir`        | 日志保存路径。                                                                                                                                                                                |
...
| `release(): void`                                                                        | 释放用例。                     |
| `logWrite2(logInfo: XLoggerInfo, log: string): boolean`                                  | 通用写入日志接口。                 |
...
| `logWrite(logInfo: XLoggerInfo, log: string): boolean`                                   | 静态方法。关闭默认Appender。        |
