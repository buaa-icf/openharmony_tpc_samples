...
NapiHandler::NapiHandler(const napi_env &env, napi_callback_info &info, int argn) : env_(env), argc_(argn)
{
    ...
    NapiCall(env_, napi_get_cb_info(env, info, &argc_, argv_, &thisArg_, &data_));
    NapiAssert(env_, argc_ >= argn, "Wrong number of arguments");
}
...
{
    napi_value val;
    NapiCall(env_, napi_get_undefined(env_, &val));
    return val;
}
...
napi_value NapiHandler::CreateArray(int len, const std::function<napi_value(int)> &doArray)
{
    ...
    NapiCall(env_, napi_create_array_with_length(env_, len, &result));
    for (int i = 0; i < len; ++i) {
        napi_value obj = doArray(i);
        NapiCall(env_, napi_set_element(env_, result, i, obj));
    }
    return result;
}
...
{
    int32_t result = -1;
    NapiCall(env_, napi_get_value_int32(env_, arg, &result));
    return result;
}
...
{
    uint32_t result = -1;
    NapiCall(env_, napi_get_value_uint32(env_, arg, &result));
    return result;
}
...
{
    int64_t result = -1;
    NapiCall(env_, napi_get_value_int64(env_, arg, &result));
    return result;
}
...
uint64_t NapiHandler::ParseArg<uint64_t>(const napi_value &arg) const
{
    uint64_t result = -1;
    bool lose = false;
    NapiCall(env_, napi_get_value_bigint_uint64(env_, arg, &result, &lose));
    return result;
}
...
{
    double result = 0;
    NapiCall(env_, napi_get_value_double(env_, arg, &result));
    return result;
}
...
{
    bool result = false;
    NapiCall(env_, napi_get_value_bool(env_, arg, &result));
    return result;
}
...
{
    bool is_arraybuffer;
    NapiCall(env_, napi_is_arraybuffer(env_, arg, &is_arraybuffer));
    if (!is_arraybuffer) {
    ...
    NapiCall(env_, napi_get_arraybuffer_info(env_, arg, &data, &byte_length));
    if (byte_length <= 0) {
        return {};
    }
...
napi_ref NapiHandler::ParseArg<napi_ref>(const napi_value &arg) const
{
    ...
    NapiCall(env_, napi_create_reference(env_, arg, 1, &result));
    return result;
}
...
{
    napi_value val;
    NapiCall(env_, napi_get_boolean(env_, v, &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_create_int32(env_, v, &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_create_int64(env_, v, &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_create_int64(env_, static_cast<int64_t>(v), &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_create_double(env_, v, &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_create_string_utf8(env_, v.c_str(), v.length(), &val));
    return val;
}
...
{
    napi_value val;
    NapiCall(env_, napi_get_reference_value(env_, v, &val));
    return val;
}
...
napi_value NapiHandler::GetNapiValue<ArrayBuffer>(const ArrayBuffer &v)
{
    ...
    NapiCall(env_, napi_create_arraybuffer(env_, v.size(), &bufferPtr, &val));
...
{
    bool result = false;
    NapiCall(env_, napi_has_named_property(env_, argv_[index], prop.c_str(), &result));
    return result;
}

napi_value NapiHandler::BindObject(void *object, napi_finalize destructor)
{
    NapiCall(env_, napi_wrap(env_, thisArg_, object, destructor, nullptr, nullptr));
    return thisArg_;
}
...
{
    napi_value result;
    NapiCall(env_, napi_create_object(env_, &result));
    return result;
}
