...
sanitize-htm 提供了HTML清理API，支持HTML片段清理。内置默认的标签、属性等、可根据默认属性进行HTML清理，同时也可进行自行配置，根据用户需求进行自定义HTML清理规则。
...
OpenHarmony npm 环境配置等更多内容，请参考 [如何安装OpenHarmony npm包](https://gitee.com/openharmony-tpc/docs/blob/master/OpenHarmony_npm_usage.md) 。
...
结合polyfill方案，兼容node.js实现

请参考 [polyfill方案地址](https://gitee.com/openharmony-sig/openharmony-polyfill) 。
