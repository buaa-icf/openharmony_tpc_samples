...
关于pinyin-pro更多的使用方式，可以参考官方文档[pinyin-pro](https://pinyin-pro.cn/guide/compare.html)

## 接口说明

| 接口                                                                     | 描述                          |
|------------------------------------------------------------------------|-----------------------------|
| **pinyin(text: string, options?: OptionsReturnString)**                | 返回转换后的信息             |
| **match(text: string, pinyin: string， options?: MatchOptions)** | 匹配成功返回匹配汉字对应下标数组; 不成功返回 null |
| **convert(pinyin, options?: ConvertOptions)**                     | 将拼音的格式进行转换，并返回转换后的拼音        |
| **customPinyin(map: object)**                                          | 支持用户自定义设置词句拼音，当中文中匹配用户自己定义的词句拼音时，优先使用用户自定义的拼音。 |
| **html(text: string, options?: OptionsReturnString)**                  | 获取<ruby></ruby> 格式的汉字和拼音 HTML 字符串      |
| **polyphonic(text: string, options?: OptionsReturnString)**            | 获取多音字的全部读音|
...
本项目基于 [Apache License 2.0](https://gitcode.com/openharmony-tpc/openharmony_tpc_samples/blob/master/pinyinProDemo/LICENSE) 协议，请自由地享受和参与开源。
